...
export default {
  ...
  data() {
    const kinerjaTahunList = []

    for (let i = 1; i <= 5; i++) {
      kinerjaTahunList.push({
        value: i,
        text: `Tahun ${this.$helper.getTahunMulai() + i - 1}`,
      })
    }
    ...
    return {
      ...
      table: {
        fields: [
          { key: 'no', label: 'No' },
          { key: 'sasaran_strategis_satker', label: 'Sasaran' },
          { key: 'iku', label: 'Indikator' },
  ...
  methods: {
    async getData(page = 1) {
      if (!this.filter.satuan_kerja_id || !this.filter.tahun) {
        return
      }
      ...
      try {
        ...
        const { data } = await axios.get('/sasaran-strategis-pd/validasi', {
          params: {
            page,
            ...this.filter,
          },
    ...
    async validate(id, status) {
      try {
        ...
        const data = this.data.data.find(item => item.id === id).validasi_tahunan[this.filter.tahun]
  
        await axios.post('/sasaran-strategis-pd/validasi', {
          id,
          status,
    ...
    async validateAll() {
      const notValidated = this.data.data.filter(item => item.validasi_tahunan[this.filter.tahun].status === null)

      try {
        this.isBusy.validateAll = true

        await Promise.all(notValidated.map(async item => {
          const data = notValidated.find(d => d.id === item.id).validasi_tahunan[this.filter.tahun]
...
<template>
  <b-card>
    <b-row>
      ...
      <b-col>
        <b-form-group label="Tahun" label-class="font-weight-bold">
          <b-form-select v-model="filter.tahun" :options="kinerjaTahunList"></b-form-select>
        </b-form-group>
      </b-col>
      <b-col>
        <b-form-group label="Cari" label-class="font-weight-bold">
          <b-form-input v-model="tempSearch" placeholder="Cari Sasaran/Indikator" />
        </b-form-group>
      </b-col>
    </b-row>
    ...
    <b-table responsive hover :fields="table.fields" :items="data.data" :busy="isBusy.getData" show-empty class="table-bordered mt-4" head-variant="info">
      ...
      <template #cell(target)="row">
        {{ row.item[`target_${filter.tahun}`] }} {{ row.item.satuan }}
      </template>
      <template #cell(realisasi)="row">
        <span v-if="row.item[`realisasi_${filter.tahun}`]">
          {{ row.item[`realisasi_${filter.tahun}`] }} {{ row.item.satuan }}
        </span>
        <span v-else>-</span>
      </template>
      <template #cell(action)="row">
        <template v-if="row.item.validasi_tahunan[filter.tahun].status === null">
          <b-form-textarea
            v-model="row.item.validasi_tahunan[filter.tahun].catatan"
            placeholder="Catatan"
            rows="3"
          ></b-form-textarea>
        ...
        <div v-else>
          <b-badge v-if="row.item.validasi_tahunan[filter.tahun].status" variant="success">Diterima</b-badge>
          <b-badge v-else variant="danger">Ditolak</b-badge>
          <div v-if="row.item.validasi_tahunan[filter.tahun].catatan" class="mt-2">
            <b>Catatan:</b> <div style="white-space: pre;">{{ row.item.validasi_tahunan[filter.tahun].catatan }}</div>
          </div>
        </div>
      </template>
    </b-table>
