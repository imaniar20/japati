  ...
  methods: {
    async getData(page = 1) {
      ...
      try {
        ...
        const { data } = await axios.get('/sasaran-strategis-pd/validasi-pengampu', {
          params: {
            page,
            ...this.filter,
          },
    ...
    async validate(id, status) {
      try {
        ...
        await axios.post('/sasaran-strategis-pd/validasi-pengampu', {
          id,
          status,
  ...
  <b-card>
    ...
    <b-table responsive hover :fields="table.fields" :items="data.data" :busy="isBusy.getData" show-empty class="table-bordered mt-4" head-variant="info">
      ...
      <template #cell(pengampu)="row">
        Kepala {{ row.item.satuan_kerja?.satuan_kerja_nama || '-' }}
      </template>
