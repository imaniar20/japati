...
export default {
  ...
  async asyncData() {
    const { data } = await axios.get('/display-mikro/renstra')
  ...
  methods: {
    ...
    async exportExcel() {
      ...
      try {
        const { data } = await axios.get('/display-mikro/renstra/export', {
          params: {
            filter: this.filter,
          },
          responseType: 'blob'
        ...
        link.href = url
        link.setAttribute('download', 'Renstra.xlsx')
    ...
    <b-table-simple  sticky-header="calc(100vh - 200px)" :aria-busy="isBusy.doFilter" hover responsive bordered striped>
      <b-thead class="text-center align-middle" head-variant="info">
        <b-tr>
          ...
          <b-th class="align-middle" colspan="5">Target Tahun ke-</b-th>
        </b-tr>
        <b-tr>
          <b-th style="top:49px;" v-for="n of 5" :key="n">{{ n }}</b-th>
        </b-tr>
      </b-thead>
      <b-tbody>
        <b-tr class="text-center" v-if="!data.data.length">
          <b-td :colspan="$role.isSuper() || $role.isSetda() ? 9 : 8">There are no records to show</b-td>
        </b-tr>
        <b-tr v-else v-for="(item, index) of data.data" :key="item.id">
          ...
          <b-td>{{ item.iku }}</b-td>
          <b-td v-for="n of 5" :key="n">{{ item[`target_${n}`] }}</b-td>
        </b-tr>
      </b-tbody>
    </b-table-simple>
