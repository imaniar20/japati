...
export default {
  ...
  async asyncData() {
    const { data } = await axios.get('/display-mikro/program-inovatif')
  ...
  methods: {
    ...
    async exportExcel() {
      ...
      try {
        const { data } = await axios.get('/display-mikro/program-inovatif/export', {
          params: {
            filter: this.filter,
          },
          responseType: 'blob'
        ...
        link.href = url
        link.setAttribute('download', 'Program Inovatif.xlsx')
    ...
    <b-table-simple sticky-header="calc(100vh - 200px)" :aria-busy="isBusy.doFilter" hover responsive bordered striped>
      <b-thead class="text-center align-middle" head-variant="info">
        <b-tr>
          ...
          <b-th class="align-middle"><span style="min-width:120px;display:inline-block;">% Capaian Kinerja</span></b-th>
          <b-th class="align-middle"><span style="min-width:275px;display:inline-block;">Program/Kegiatan/Langkah Aksi Inovatif</span></b-th>
        </b-tr>
        <b-tr>
          <b-th style="top:56px" v-for="n of $role.isSuper() || $role.isSetda() ? 6 : 5" :key="n">{{ n }}</b-th>
        </b-tr>
      </b-thead>
      <b-tbody>
