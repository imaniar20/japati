...
export default {
  ...
  async asyncData() {
    const { data } = await axios.get('/display-mikro/rkt')
  ...
  methods: {
    ...
    async exportExcel() {
      ...
      try {
        const { data } = await axios.get('/display-mikro/rkt/export', {
          params: {
            filter: this.filter,
          },
          responseType: 'blob'
        ...
        link.href = url
        link.setAttribute('download', 'Rkt.xlsx')
  ...
  <b-card>
    ...
    <b-table-simple sticky-header="calc(100vh - 200px)" :aria-busy="isBusy.doFilter" hover responsive bordered striped>
      <b-thead class="text-center align-middle" head-variant="info">
        ...
        <b-tr>
          <b-th style="top:45px" v-for="n of $role.isSuper() || $role.isSetda() ? 9 : 8" :key="n">{{ n }}</b-th>
        </b-tr>
      </b-thead>
      <b-tbody>
        ...
        <b-tr v-else v-for="(item, index) of data.data" :key="item.id">
          ...
          <b-td>
            <span v-if="item.kinerja_program">{{ item.kinerja_program.anggaran | rupiah }}</span>
            <span v-else>-</span>
          </b-td>
        </b-tr>
      </b-tbody>
