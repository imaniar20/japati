  ...
  methods: {
    async getData(page = 1) {
      ...
      try {
        ...
        const { data } = await axios.get('/kinerja-kegiatan/validasi-pengampu', {
          params: {
            page,
            ...this.filter,
          },
    ...
    async validate(id, status) {
      try {
        ...
        await axios.post('/kinerja-kegiatan/validasi-pengampu', {
          id,
          status,
    ...
    <b-row>
      ...
      <b-col>
        <b-form-group label="Cari" label-class="font-weight-bold">
          <b-form-input v-model="tempSearch" placeholder="Cari Kegiatan/Sasaran/Indikator" />
        </b-form-group>
      </b-col>
    </b-row>
