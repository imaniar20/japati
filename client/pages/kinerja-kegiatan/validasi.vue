  ...
  data() {
    return {
      ...
      table: {
        fields: [
          { key: 'no', label: 'No' },
          { key: 'kinerja_program', label: 'Sasaran Program' },
  ...
  methods: {
    async getData(page = 1) {
      ...
      try {
        ...
        const { data } = await axios.get('/kinerja-kegiatan/validasi', {
          params: {
            page,
            ...this.filter,
          },
    ...
    async validate(id, status) {
      try {
        ...
        await axios.post('/kinerja-kegiatan/validasi', {
          id,
          status,
...
<template>
  <b-card>
    <b-row>
      ...
      <b-col>
        <b-form-group label="Cari" label-class="font-weight-bold">
          <b-form-input v-model="tempSearch" placeholder="Cari Sasaran Program/Sasaran Kegiatan/Indikator Kegiatan" />
        </b-form-group>
      </b-col>
    </b-row>
    ...
    <b-table responsive hover :fields="table.fields" :items="data.data" :busy="isBusy.getData" show-empty class="table-bordered mt-4" head-variant="info">
      ...
      <template #cell(kinerja_program)="row">
        {{ row.value.sasaran || '-' }}
      </template>
