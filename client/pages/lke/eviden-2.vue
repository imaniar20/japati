    ...
    <b-table-simple class="mt-3 pb-5" responsive bordered hover>
      <b-thead>
        <b-tr>
          ...
          <b-th>Jawaban</b-th>
          <b-th>Eviden</b-th>
          <b-th>Status Penilaian Sebelumnya</b-th>
          <b-th>Catatan Penilaian Sebelumnya</b-th>
      ...
      <b-tbody v-if="isReady">
        <template v-for="(komponen, komponenIndex) in komponenList">
          <b-tr :key="komponen.id">
            <b-td :colspan="showHasilPenilaian ? 8 : 6"><h5>{{ komponen.nomor }}. {{ komponen.nama }}</h5></b-td>
          ...
          <template v-for="(subKomponen, subKomponenIndex) in komponen.sub_komponen">
            ...
            <b-tr v-for="kriteria in subKomponen.kriteria" :key="kriteria.id" :class="{'bg-pink': showHasilPenilaian && (form[kriteria.id].parameter_id != getLastRiwayat(kriteria.eviden.riwayat).nilai || !getLastRiwayat(kriteria.eviden.riwayat).status) || (!showHasilPenilaian && getLastRiwayat(kriteria.eviden.riwayat_penilaian_sebelumnya).status === false)}">
              ...
              <b-td>
                <b>{{ getLastRiwayat(kriteria.eviden.riwayat_penilaian_sebelumnya).status === true ? "Diterima" : "Ditolak" }}</b>
              </b-td>
              <b-td>
                <b>{{  getLastRiwayat(kriteria.eviden.riwayat_penilaian_sebelumnya).catatan }}</b>
              </b-td>
