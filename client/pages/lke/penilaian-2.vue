          ...
          <b-tr>
            ...
            <b-th class="text-center align-middle">Catatan</b-th>
             <b-th class="text-center align-middle"
              >Terima/Tolak Penilaian Sebelumnya</b-th
            >
            <b-th class="text-center align-middle"
              >Catatan Penilaian Sebelumnya</b-th
            >
          </b-tr>
        </b-thead>
        <b-tbody v-if="isReady">
            <b-tr>
        <b-td colspan="8" class="text-left text-primary">
          ...
          <template v-for="(komponen, komponenIndex) in komponenList">
            <b-tr :key="komponen.id">
              <b-td colspan="8"><h5>{{ komponen.nomor }}. {{ komponen.nama }}</h5>
               <p class="text-primary">Total Nilai Komponen: {{
            ...
            <template v-for="(subKomponen, subKomponenIndex) in komponen.sub_komponen">
              <b-tr :key="subKomponen.id">
                ...
                <b-td colspan="8"><h6>{{ subKomponen.nama }}</h6>
                <p class="text-primary" >Total Nilai Sub Komponen: {{
              ...
              <b-tr v-for="(kriteria, kriteriaIndex) in subKomponen.kriteria" :key="kriteria.id" :class="{'bg-pink': form[getLastRiwayat(kriteria.eviden.riwayat).id].is_revisi || form[getLastRiwayat(kriteria.eviden.riwayat).id].nilai != getLastRiwayat(kriteria.eviden.riwayat).parameter_id  ||  form[getLastRiwayat(kriteria.eviden.riwayat).id].status == false, }">
                ...
                <b-td style="min-width: 300px;">
                 ...
                 <b-td>
                  <b :class="{'text-danger' : getLastRiwayat(kriteria.eviden.riwayat_penilaian_sebelumnya)
                      .status !== true }">{{
                    getLastRiwayat(kriteria.eviden.riwayat_penilaian_sebelumnya)
                      .status === true
                      ? "Diterima"
                      : "Ditolak"
                  }}</b>
