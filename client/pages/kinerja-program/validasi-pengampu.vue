  ...
  methods: {
    async getData(page = 1) {
      ...
      try {
        ...
        const { data } = await axios.get('/kinerja-program/validasi-pengampu', {
          params: {
            page,
            ...this.filter,
          },
    ...
    async validate(id, status) {
      try {
        ...
        await axios.post('/kinerja-program/validasi-pengampu', {
          id,
          status,
    ...
    <b-row>
      ...
      <b-col>
        <b-form-group label="Cari" label-class="font-weight-bold">
          <b-form-input v-model="tempSearch" placeholder="Cari Program/Sasaran/Indikator" />
        </b-form-group>
      </b-col>
    </b-row>
