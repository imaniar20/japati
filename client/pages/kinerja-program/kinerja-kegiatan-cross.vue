  ...
  methods: {
    async getData(page = 1) {
      try {
        ...
        const { data } = await axios.get(`kinerja-program/${this.id}/kinerja-kegiatan-cross`, {
          params: {
            filter: this.filter,
            page,
          }
    ...
    async set(kinerjaKegiatanId, isSet) {
      try {
        this.isBusy.set = kinerjaKegiatanId

        await axios.post(`kinerja-program/${this.id}/kinerja-kegiatan-cross`, {
          set: isSet ? 1 : 0,
...
<template>
  <b-card>
    <b-row>
      ...
      <b-col>
        <FilterSasaranKinerja v-model="filter.sasaran" model="kinerja-kegiatan" :satker="filter.satuan_kerja_id" />
        <FilterIndikatorKinerja v-model="filter.indikator" model="kinerja-kegiatan" :satker="filter.satuan_kerja_id" />
      </b-col>
    </b-row>
    ...
    <b-table responsive hover striped :fields="table.fields" :items="data.data" :busy="isBusy.getData" show-empty class="table-bordered mt-4" head-variant="info">
      ...
      <template #cell(action)="row">
        <b-button v-if="!row.item.kinerja_kegiatan_cross_exists" @click="set(row.item.id, true)" variant="success" size="sm" class="rounded-circle" v-b-tooltip.hover title="Tambahkan" :disabled="isBusy.set == row.item.id">
      ...
      <template #cell(keterangan)="row">
        <span v-if="row.item.kinerja_kegiatan_cross_exists">Sudah ditandai sebagai cross cutting</span>
      </template>
    </b-table>
