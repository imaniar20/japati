...
export default {
  ...
  async asyncData({ params }) {
    const [{ data: form }, { data: roles }, { data: satuanKerja }] = await Promise.all([
      axios.get(`admin/users/${params.id}`),
  ...
  methods: {
    async update() {
      try {
        ...
        await axios.patch(`admin/users/${this.form.id}`, this.form)
...
<template>
  <b-card>
    <form @submit.prevent="update()">
      ...
      <b-form-group label-cols="12" label-cols-md="2" label="Password Baru" label-class="font-weight-bold" label-for="password">
        <b-form-input id="password" v-model="form.password" type="password" minlength="6"></b-form-input>
      </b-form-group>
