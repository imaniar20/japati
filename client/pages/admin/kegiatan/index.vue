  ...
  methods: {
    ...
    async destroySelected() {
      ...
      const result = await Swal.fire({
        ...
        text: `Akan menghapus ${this.selectedIds.length} data kegiatan terpilih`,
        type: 'warning',
      ...
      if (result.value) {
        try {
          const { data } = await axios.post('kegiatan-data/destroy-batch', {
    ...
    async destroyAllData() {
      ...
      const result = await Swal.fire({
        ...
        text: `Apakah Anda yakin ingin menghapus SELURUH (${this.filteredData.length}) data kegiatan untuk tahun ${this.filter.tahun_kinerja}?`,
        type: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#bd2130',
      ...
      if (result.value) {
        try {
          const { data } = await axios.delete('kegiatan-data/destroy-all', {
            data: {
              tahun_kinerja: this.filter.tahun_kinerja,
            },
          })
