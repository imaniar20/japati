  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'kegiatan_count', label: 'Kegiatan' },
  ...
  <b-card>
    ...
    <b-row class="align-items-end mb-3">
      ...
      <b-col cols="12" :md="$role.isSuper() ? 2 : 3" class="text-md-right">
        <b-badge variant="info" class="px-3 py-2">{{ filteredData.length }} Program</b-badge>
      </b-col>
    </b-row>
    ...
    <b-table responsive hover striped :busy="isBusy.getData" :items="filteredData" :fields="fields" show-empty class="table-bordered" head-variant="info">
      ...
      <template #cell(kegiatan_count)="row">
        <b-badge :variant="row.item.kegiatan_count ? 'warning' : 'secondary'" class="px-3 py-2">
          {{ row.item.kegiatan_count || 0 }}
        </b-badge>
      </template>
