...
export default {
  ...
  methods: {
    async getData(page = 1) {
      ...
      const { data } = await axios.get('/lkip/narasi-pemda', {
        params: {
          page,
          filter: this.filter,
        }
    ...
    destroy(index, id) {
      doDestroy({
        preConfirm: async () => {
          return axios.delete(`/lkip/narasi-pemda/${id}`)
            .then(() => {
...
<template>
  <b-card>
    <div class="text-right">
      <nuxt-link v-if="$role.isPemerintahDaerah()" to="/lkip/narasi-pemda/create" class="btn btn-primary">
    ...
    <b-table-simple class="mt-3" :aria-busy="isBusy" responsive bordered striped hover>
      ...
      <b-tbody>
        <b-tr class="text-center" v-if="isBusy && !data.data.length">
          <b-td :colspan="$role.isSuper() ? 6 : 5"><b-spinner small></b-spinner> Sedang memuat data...</b-td>
        </b-tr>
        <b-tr class="text-center" v-else-if="!data.data.length">
          <b-td :colspan="$role.isSuper() ? 6 : 5">There are no records to show</b-td>
        </b-tr>
        <b-tr v-else v-for="(item, index) of data.data" :key="item.id">
          <b-td>{{ data.from + index }}</b-td>
          <b-td style="max-width:150px;" v-if="$role.isSuper()">{{ item.satuan_kerja.satuan_kerja_nama }}</b-td>
          <b-td style="max-width:150px;">{{ item.sasaran_strategis.sasaran }}</b-td>
          <b-td style="max-width:150px;">{{ item.indikator_sasaran_strategis.indikator }}</b-td>
          ...
          <b-td>
            <div class="text-nowrap">
              <nuxt-link :to="`/lkip/narasi-pemda/${item.id}`" class="btn btn-outline-success btn-sm m-1 rounded-circle" title="Detail">
                <i class="fa fa-eye" aria-hidden="true"></i>
              </nuxt-link>
              <nuxt-link v-if="$role.isPemerintahDaerah()" :to="`/lkip/narasi-pemda/${item.id}/edit`" class="btn btn-outline-warning btn-sm m-1 rounded-circle" title="Edit">
                <i class="fa fa-pencil" aria-hidden="true"></i>
              </nuxt-link>
              <b-button v-if="$role.isPemerintahDaerah()" @click="destroy(index, item.id)" variant="outline-danger" size="sm" class="m-1 rounded-circle" title="Hapus">
