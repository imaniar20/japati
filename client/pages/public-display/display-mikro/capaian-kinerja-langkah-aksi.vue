...
export default {
  ...
  methods: {
    async doFilter(page = 1) {
      ...
      const { data } = await axios.get('/public-display/display-mikro/capaian-kinerja-langkah-aksi', {
        params: {
          filter: this.filter,
          page,
        }
  ...
  <b-card>
    ...
    <b-table-simple sticky-header="calc(100vh - 200px)" :aria-busy="isBusy" hover responsive bordered striped>
      <b-thead class="text-center align-middle" head-variant="info">
        ...
        <b-tr>
          <b-th style="top:98px" v-for="n of 23" :key="n">{{ n }}</b-th>
          <!-- <b-th style="top:110px" v-for="n of 27" :key="n">{{ n }}</b-th> -->
        </b-tr>
      </b-thead>
      <b-tbody>
        <b-tr class="text-center" v-if="!data.data.length">
          <b-td colspan="27">There are no records to show</b-td>
        </b-tr>
