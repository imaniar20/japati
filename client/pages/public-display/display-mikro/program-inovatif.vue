...
export default {
  ...
  methods: {
    async doFilter(page = 1) {
      ...
      const { data } = await axios.get('/public-display/display-mikro/program-inovatif', {
        params: {
          filter: this.filter,
          page,
        }
  ...
  <b-card>
    ...
    <b-table-simple sticky-header="calc(100vh - 200px)" :aria-busy="isBusy" hover responsive bordered striped>
      <b-thead class="text-center align-middle" head-variant="info">
        ...
        <b-tr>
          <b-th style="top:56px" v-for="n of 6" :key="n">{{ n }}</b-th>
        </b-tr>
      </b-thead>
      <b-tbody>
        <b-tr class="text-center" v-if="!data.data.length">
          <b-td colspan="6">There are no records to show</b-td>
        </b-tr>
