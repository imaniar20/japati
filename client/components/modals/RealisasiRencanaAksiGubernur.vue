...
export default {
  ...
  mounted() {
    this.$nuxt.$on(this.showEventId, (satuanKerjaId, sasaranStrategisRpjmdId, iku, sasaran_strategis) => {
      this.satuanKerjaId = satuanKerjaId
      this.sasaranStrategisRpjmdId = sasaranStrategisRpjmdId
      this.iku = iku
      this.sasaran_strategis = sasaran_strategis
      this.reset()
      this.getData()
      ...
      <b-tbody>
        ...
        <template v-else>
          <b-tr v-for="(rencanaAksi, index) in data.data" :key="index">
            <b-td v-if="index == 0" :rowspan="data.data.length">{{ sasaran_strategis }}</b-td>
