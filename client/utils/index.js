...
export function parseParams (params) {
  if (!params) return {}
  try {
    return JSON.parse(decode(params))
  } catch (e) {
    return {}
  }
}
...
export function randSeed(min, max, seed) {
  return Math.round(min + (parseInt(md5(seed), 16) / parseInt("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF", 16)) * (max - min));
}
