...
class SyncMasterDataKegiatan extends Command
{
     ...
     * @var string
     */
    protected $signature = 'sync:master-kegiatan
     ...
     * @var string
     */
    protected $description = 'Sync master data kegiatan';
    ...
    public function handle()
    {
        ...
        $dataOpd = DB::connection('siap')
            ->select('SELECT DISTINCT
                db.kode_giat AS kode,
                db.nama_giat AS nama,
                db.kode_program AS program_kode,
        ...
        foreach ($groupSatuanKerja as $satuanKerjaId => $data) {
            $programList = Program::tahunKinerja()
            ...
            foreach ($data as $item) {
                $update = [
                    ...
                    'program_id' => $programList->where('kode', $item->program_kode)->first()->id,
                ];

                unset($item->program_kode);

                $create = array_diff_assoc((array) $item, $update);

                Kegiatan::updateOrCreate($update, $create);
                $bar->advance();
            }
        }
